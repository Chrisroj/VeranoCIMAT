...
from criptos import create_corr_plot
...
# gjrGARCH models
#image_directory_garch = os.path.join(path, "7_gjrGARCH_regressor")
#list_of_images_garch = [img.split(".")[0] for img in os.listdir(os.path.join(path, "7_gjrGARCH_regressor"))]
#static_image_route_garch = '/static/'

# gjrGARCH coeficientes
garch_coef = pd.read_csv(os.path.join(path, "7_garch_coef.csv"))
garch_coef.columns = ['Coef', ' Estimate', ' Std. Error', ' t value', 'Pr(>|t|)']

# Forecast 100 días
garch_forecast = pd.read_csv(os.path.join(path, "8_garch_sigma.csv"), index_col = "Fecha")
...
app.layout = html.Div(children=[
    ...
    # Tweets Mensuales
    html.H2(children='Tweets Mensuales por Usuarios'),
   ...
   # Bar of Frequencies
    ...
    dcc.Dropdown(
        id='image-dropdown',
        options=[{'label': i, 'value': i} for i in list_of_images if i[0] != "1"],
        value="binance",
        clearable = False
    ),
    ...
    html.H2(children ="Modelo: gjrGARCH(1,1) - ged "),
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id='image-garch',
                options=[{'label': i, 'value': i} for i in list_of_images if i[0] == "1"],
                value="1_ACF of Squared Observations",
                clearable = False
            ),
            
            html.Img(id='image-garch-plot')
            ], style={'width': '49%', 'display': 'inline-block'}),
        
        html.Div([
             html.H2(children ="Coeficientes del Modelo"),
             
            # GARCH coef
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in garch_coef.columns],
                data=garch_coef.to_dict('records'),
                )
            ], style={'width': '49%', 'float': 'right', 'display': 'inline-block'})
        
        ]),
    
    html.H2(children = "Pronóstico de la volatilidad 100 Días"),
    
    dcc.Graph(
        id = 'garch-forecast',
        figure = garch_forecast.plot()
              )  
])
...
@app.callback(
    dash.dependencies.Output('image-garch-plot', 'src'),
    [dash.dependencies.Input('image-garch', 'value')])
def update_image_src_garch(value):
    return static_image_route + value + ".png"
